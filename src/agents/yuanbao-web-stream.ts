...
import {
  YuanbaoWebClientBrowser,
  type YuanbaoWebClientOptions,
} from "../providers/yuanbao-web-client-browser.js";

const conversationMap = new Map<string, string>();

export function createYuanbaoWebStreamFn(cookieOrJson: string): StreamFn {
  let options: YuanbaoWebClientOptions;
  try {
  ...
  const client = new YuanbaoWebClientBrowser(options);
  ...
  return (model, context, streamOptions) => {
    ...
    const run = async () => {
      try {
        ...
        if (!prompt) {
          throw new Error("No message found to send to Yuanbao API");
        }

        console.log(`[YuanbaoWebStream] Starting run for session: ${sessionKey}`);
        console.log(`[YuanbaoWebStream] Conversation ID: ${conversationId || "new"}`);
        console.log(`[YuanbaoWebStream] Prompt length: ${prompt.length}`);
        ...
        if (!responseStream) {
          throw new Error("Yuanbao API returned empty response body");
        }
        ...
        console.log(`[YuanbaoWebStream] Stream completed. Content length: ${accumulatedContent.length}`);
